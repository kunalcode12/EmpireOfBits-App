...
import { normalizeBackendMove, parseFen, toAlgebraic, type ChessMove, type Color } from '../utils/chessHelpers';
...
const reducer = (state: GameState, action: GameAction): GameState => {
  switch (action.type) {
    ...
    case 'CHECK':
      return { ...state, checkSquare: null, toast: 'Check' };
    case 'DRAW_OFFER':
    ...
    case 'GAME_OVER': {
      const payloadResult = action.payload.result;
      ...
      return { ...state, phase: 'finished', result, drawOfferIncoming: false, toast: action.payload.message ?? null };
    }
    case 'TOAST':
...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<GameContextValue>(
    ...
    [state, startMatchmaking, cancelMatchmaking, makeMove, resignGame, offerDrawGame, acceptDrawOffer, rejectDrawOffer, resetGame, clearToast, sendChatMessage],
  );
